...
function Get-Stats-Realtime {
    ...
    while ($elapsed -lt ($Duration + 1)) {
        ...
        $stats = Get-Stat -Entity $Entity -Stat mem.consumed.average -Memory -Cpu -Disk -Network -Realtime -Start $start | Select-Object Timestamp,Entity,MetricId,Instance,Value,Unit
